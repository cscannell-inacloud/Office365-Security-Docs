...
|X-CustomSpam: [ASFOption]  <br/> |The message matched an advanced spam filtering (ASF) option. For example, **X-CustomSpam: Image links to remote sites** denotes that the **Image links to remote sites** ASF option was matched. To find out which X-header text is added for each specific ASF option, see [Advanced spam filtering (ASF) options](advanced-spam-filtering-asf-options.md).  <br/> |
