...
- Beginning in late March 2018, ATP Safe Links protection is extended to apply to URLs in Office Online (Word Online, Excel Online, PowerPoint Online, and OneNote Online) and Office 365 ProPlus on Mac.
...
1. If the URL is to a website that is included in a [custom "Do not rewrite" URLs list](set-up-a-custom-do-not-rewrite-urls-list-with-atp.md) for a policy that applies to the user, the website opens. 
    
2. If the URL is to a website that is included in the organization's [custom blocked URLs list](set-up-a-custom-blocked-urls-list-wtih-atp.md), a [warning page](atp-safe-links-warning-pages.md) opens. 
    
3. If the URL is to a website that has been determined to be malicious, a [warning page](atp-safe-links-warning-pages.md) opens. 
    
4. If the URL goes to a downloadable file and your organization's [ATP Safe Links policies](set-up-atp-safe-links-policies.md) are configured to scan such content, the downloadable file is checked. 
    
5. If the URL is determined to be safe, the website opens.
    
## ATP Safe Links in Office documents - how it works
<a name="atpforoffice"> </a>

At a high level, here's how ATP Safe Links protection works for URLs in Office 365 ProPlus applications (current versions of Word, Excel, and PowerPoint on Windows or Mac, Office apps on iOS or Android devices, and Visio on Windows):
  
1. People have installed Office 365 ProPlus on their computer, smartphone, or tablet.
    
2. A user opens a Word, Excel, PowerPoint, or Visio, and is signed in to Office 365 Enterprise using their work or school account. The document contains URLs.
...
- **The organization's email is hosted in Office 365**, not in an on-premises server. 
