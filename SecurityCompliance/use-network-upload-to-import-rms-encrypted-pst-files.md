...
[Step 1: Set up Azure Rights Management for PST Import ](use-network-upload-to-import-rms-encrypted-pst-files.md#step1)
  
[Step 2: Generate an encryption key for PST Import](use-network-upload-to-import-rms-encrypted-pst-files.md#step2)
  
[Step 3: Obtain RMS tenant ID and licensing URL](use-network-upload-to-import-rms-encrypted-pst-files.md#step3)
  
[Step 4: Download the PST Import tools and copy the SAS URL](use-network-upload-to-import-rms-encrypted-pst-files.md#step4)
  
[Step 5: Encrypt and upload your PST files to Office 365](use-network-upload-to-import-rms-encrypted-pst-files.md#step5)
  
[(Optional) Step 6: View a list of the PST files uploaded to Office 365](use-network-upload-to-import-rms-encrypted-pst-files.md#step6)
  
[Step 7: Create the PST Import mapping file](use-network-upload-to-import-rms-encrypted-pst-files.md#step7)
  
[Step 8: Create a PST Import job in Office 365](use-network-upload-to-import-rms-encrypted-pst-files.md#step8)
...
For more information about importing data to Office 365, see [Overview of importing PST files to Office 365](https://support.office.com/article/ba688e0a-0fcb-4bd7-8e57-2b669564ea84).
...
[Return to top](use-network-upload-to-import-rms-encrypted-pst-files.md#top)
...
- [Activate Azure RMS](use-network-upload-to-import-rms-encrypted-pst-files.md#activate)
    
- [Configure RMS in Exchange Online](use-network-upload-to-import-rms-encrypted-pst-files.md#configureRMS)
    
- [Install the Active Directory RMS Client ](use-network-upload-to-import-rms-encrypted-pst-files.md#rmsclient)
...
### Configure RMS in Exchange Online
<a name="configureRMS"> </a>
...
### Install the Active Directory RMS Client
<a name="rmsclient"> </a>
...
9. In the pop-up window, click **Run** to install the Azure AzCopy tool. 
    ...
    > [!IMPORTANT]
    > Be sure to install the Azure AzCopy tool in the default location (%ProgramFiles(x86)%\Microsoft SDKs\Azure\ AzCopy on a computer running 64-bit Windows). That's because when you run the O365ImportTool.exe in Step 5, it looks for the AzCopy tool in this location. 
...
- Verify that PST files from the shared folder or file server in your organization were successfully uploaded to the Azure blob, and that they are encrypted. Encrypted PST files have a  `.pfile` extension appended to the PST filename; for example,  `pilarp.pst.pfile`.
...
[](use-network-upload-to-import-pst-files.md#top)
